...
test('menu: „Nowa gra” prowadzi do ustawień, „Wróć” z powrotem; scena w stylu pikselowym', async () => {
  ...
  assert.deepEqual(r.labels, ['Nowa gra', 'Wczytaj grę', 'Najlepsze wyniki', 'Dźwięk', 'Twórcy', 'Wyjście']);
  ...
  await frames(page, 3);
  await page.evaluate(() => G.screens.setup.buttons.find(b => b.label === 'Wróć').action());
  await page.waitForFunction(() => G.screenName === 'menu');
});
